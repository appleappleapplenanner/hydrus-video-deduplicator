...
class TestVptree(unittest.TestCase):
    ...
    def setUp(self):
        self.all_points = list(range(10000))
        self.hamming = lambda x, y: bin(x ^ y).count("1")
        self.tree = VPTree(
            points=self.all_points,
            dist_fn=self.hamming,
        )
    ...
    def test_empty_points_raises_valueerror(self):
        self.assertRaises(ValueError, VPTree, [], euclidean)

    def test_zero_neighbors_raises_valueerror(self):
        tree = VPTree([1, 2, 3], euclidean)
    ...
    def test_custom_points(self):
        ...
        tree = VPTree(points, custom_point_distance)
